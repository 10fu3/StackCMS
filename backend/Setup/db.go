...
func DefineTables(db *sqlx.DB) error {
	sqls := []string{
		...
		"CREATE TABLE IF NOT EXISTS apis (id VARCHAR(40) not null primary key,is_single BOOLEAN not null, preview_url VARCHAR(2000), api_id VARCHAR(40) UNIQUE)",
		"CREATE TABLE IF NOT EXISTS fields (field_id VARCHAR(40) not null primary key, api_id VARCHAR(40), field_name VARCHAR(40) UNIQUE,field_type VARCHAR(40) , priority integer not null , relation_api VARCHAR(40))",
